#include <iostream>
#include <initializer_list> // std::initializer_list を使うために必要
...
// std::initializer_list を受け取るコンストラクター
int_vector::int_vector(std::initializer_list<int> init) 
  : m_size{init.size()}, m_array{new int[m_size]} {
  std::size_t i = 0;
...
int main() {
  // 配列のようにstd::initializer_listを使って初期化する　
