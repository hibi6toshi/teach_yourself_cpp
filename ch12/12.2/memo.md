...
## 12.2.7 std::vectorの特徴
std::vectorはポインターとnew/delete[]による動的配列で実装されています。
各要素は隙間なく連続していなければならないため、挿入・削除を行うと（それ以降の）要素の移動が行われます。移動しなければならない要素数は、挿入・削除位置が先頭に近いほど多く、末尾に近いほど少なくなるため、先頭に近い位置での挿入・削除には相当の処理時間を要することになります。

末尾に対する挿入・削除はコンテナ内の要素数によらず基本的に高速であり、よく使うので末尾への挿入のためにpush_back()/emplace_back()、末尾要素の削除にはpop_back()というメンバーkな数が定義されています。
...
pv.emplace_back("bob", 18); // personのコンストラクターが呼ばれる
```

ただし、要素の挿入・削除は他の要素の異動を伴うため、ある要素を指しているイテレーターは、挿入・削除が行われると無効になることがあります。無効になったイテレーターを使って要素を参照しようとしてはいけません。参照すると、そのイテレーターを使った全ての操作でプログラムがクラッシュするなど、予期せぬ動作を起こします。
```C++
std::vector iv = { 0, 1, 2, 3 };
auto tail = iv.end() - 1; // tail は3を指す

iv.erase(iv.begin()) // 先頭要素を削除。この瞬間にtailは無効になる（３を指していない）
```