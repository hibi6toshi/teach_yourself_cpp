...
void determine_class(const Base& obj) {
  ...
  if (obj_type == typeid(DerivedA)) {
    ...
    std::cout << "obj は DerivedAでもDerivedBでもないインスタンスです" << std::endl;
  }
}

// datermine_classが知らない派生クラス
...
int main() {
  ...
  // obj は DerivedA型のインスタンスです
  // ojb は DerivedB型のインスタンスです
  // obj は DerivedAでもDerivedBでもないインスタンスです
}